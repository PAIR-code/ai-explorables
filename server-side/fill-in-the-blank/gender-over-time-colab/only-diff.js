...
async function drawSentence({s0, s1, tidyCSV, minYear}, i){
  ...
  c.svg.append('rect')
    .at({width: c.width, height: c.height/2, opacity: .1, fill: colors[0]})

  c.svg.append('rect')
    .at({width: c.width, height: c.height/2, opacity: .1, fill: colors[1], y: c.height/2})

  console.log(tidy[0])
  c.x.domain(d3.extent(tidy, d => d.year))

  var minY = d3.min(d3.extent(tidy, d => d.dif), Math.abs)
  var maxY = d3.max(d3.extent(tidy, d => d.dif), Math.abs)
  ...
  var tokenSel = c.svg.appendMany('g', byToken)
