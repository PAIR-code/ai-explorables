...
We can train a simpler model with a head start on the problem, constructing an embedding matrix that places `$a$` and `$b$` on a circle by computing `$\cos$` and `$\sin$` for each row index `$i$` of the matrix.<a class='footstart' key='unit-circle'></a>
...
By computing `$\cos(i\frac{2\pi}{67})$` and `$\sin (i\frac{2\pi}{67})$` for each row index `$i$` from 0 to 67, we represent evenly spaced values around the unit circle. Here's what `$\textbf{W}_{\text{embed}}$` looks like on the unit circle:
<span class='fn-break'></span>
