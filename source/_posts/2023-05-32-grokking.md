...
socialsummary: An interactive introduction to mechanistic interpretability and grokking. 
shareimg: TKTK
...
To lower loss without using higher weights (which would be punished by weight decay), the model uses several frequencies, taking advantage of constructive interference.<a class='citestart' key='ProgressMeasures'></a> There's nothing magical about the frequencies 4, 5, 7 and 26 — click through other training runs below to see the same algorithm get learned in different ways.  
