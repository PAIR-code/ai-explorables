window.makeAxii = function () {
  ...
  stateAxis.appendMany('path', d3.range(ages.length + 1)).at({
  ...
  var ageScale = d3
    .scaleBand()
    .domain(ages.slice().reverse())
    .range(c.x.range());
  var ageAxis = c.svg.append('g.axis.age.init-hidden');

  ageAxis
    .appendMany('text', ages)
    .translate((d) => [-30, ageScale(d) + bw])
    .text((d) => d)
    .at({dy: '.33em'})
    .st({fill: '#444'});

  ageAxis.appendMany('path', d3.range(ages.length + 1)).at({
  ...
  headAxis
    .appendMany('text.bold', ['Heads', 'Tails'])
    .text((d) => d)
  ...
  headCaptionAxis
    .appendMany('text', ['reports plagiarism', 'reports truth'])
    .text((d) => d)
    .translate((d, i) => [
      i ? (c.width / 4) * 3 + 20 : c.width / 4 - 20,
      88 + 15,
    ])
    .at({textAnchor: 'middle'})
    .st({fill: '#444'});
