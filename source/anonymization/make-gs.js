window.makeGS = function () {
  ...
  function updateSlide(i) {
    ...
    sel.student
      .transition('xKey')
      .duration(dur)
      .delay(dur ? slide.circleDelayFn : 0)
      .translate((d) =>
        d.isAdditionalStudent && slide.xKey != 'plagerizedShifted'
          ? [0, 0]
          : d.pos[slide.xKey],
      );
    ...
    sel.uniqueSeasonBox
      .transition()
      .delay((d, i) => (slide.showUniqueSeasonBox ? dur * 2 + i * 40 : 0))
      .duration(slide.showUniqueSeasonBox ? 0 : dur)
      .st({opacity: slide.showUniqueSeasonBox ? 1 : 0});

    if (
      sliders.headsProb != slide.headsProbTarget &&
      slide.animateHeadsProbSlider != -1
    ) {
    ...
    sel.flipCircle
      ...
      .at({
        transform: (d) => {
          return slide.showFlipCircle &&
            d.coinVals[estimates.active.index] < sliders.headsProb
            ? 'scale(1)'
            : 'scale(.1)';
        },
      });
