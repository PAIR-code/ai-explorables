window.makeSel = function () {
  function ttFmt(d) {
    ...
    if (slides.curSlide.index == 4) {
      ageStr =
        ageStr +
        ' born in the ' +
        ['spring', 'summer', 'fall', 'winter'][d.season];
    }
  ...
  var uniqueBox = c.svg
    .appendMany(
      ...
      students.byAgeState.filter((d) => d.length == 1),
    )
    .translate((d) => d.pos)
    .at(rectAt.ageState);
  ...
  var uniqueSeasonBox = c.svg
    .appendMany(
      'rect.unique.init-hidden',
      students.byAgeStateSeason.filter(
        (d) => d.length == 1 && d[0].group.ageState.length > 1,
      ),
    )
    .translate((d) => d.pos)
  ...
  var studentGroup = c.svg.append('g').at({width: 500, height: 500});

  var student = studentGroup
    .appendMany('g.student', students.all)
