...
d3.select('#big-matrix')
  .html('')
  .appendMany('div.big-container', [
    {w: wLarge, s: f, isText: 1},
    {w: wLarge, s: m, isText: 1},
  ])
  .each(drawMatrix);
...
function drawMetric(i) {
  ...
  sel
    .append('h3')
    .st({marginBottom: 20, fontSize: isScaled ? 30 : 20})
    .html(isScaled ? text.replace('<br>', '') : text);
  ...
  var fDiv = sel
    .append('div')
    .st({position: 'relative', top: -h + 7})
    .datum({w: 50, s: f, isText: 0, colors})
    .each(drawMatrix);
  ...
  svg.append('path').at({stroke: '#ccc', d: `M ${width / 2 + 0.5} 0 V ${h}`});

  var errorSel = svg
    .append('path')
    .translate(width / 2 + 0.5, 0)
  ...
  mSel
    .append('circle')
    .at({r: 4, cx: width / 2 + 0.5, fill: 'none', stroke: '#000'});
  fSel
    .append('circle')
    .at({r: 4, cx: width / 2 + 0.5, fill: 'none', stroke: '#000'});
  ...
  var mDiv = sel
    .append('div')
    .st({position: 'relative', top: -h + 7})
    .datum({w: 50, s: m, isText: 0, colors})
    .each(drawMatrix);
...
function drawMatrix({s, w, isText, colors}) {
  var svg = d3.select(this).append('svg').at({width: w, height: w});
  ...
  svg
    .append('rect.b')
    .at({width: bh, height: w})
    .translate(-bh / 2, 0)
    .datum('b');

  var bh = 20;
  svg
    .append('rect.drag.tpr')
    .at({height: bh})
    .translate(-bh / 2, 1)
    .call(makeDrag('tpr', 1))
    .datum('tpr')
    .call(d3.attachTooltip)
    .on('mouseover', ttFormat);

  svg
    .append('rect.drag.fnr')
    .at({height: bh})
    .translate(-bh / 2, 1)
    .call(makeDrag('fnr', 1))
    .datum('fnr')
  ...
  function ttFormat(str) {
    var html = '';
    if (str == 'tpr')
      html = `${d3.format('.0%')(s.tpr)} of sick ${s.titleStr.toLowerCase()} test positive`;
    if (str == 'fnr')
      html = `${d3.format('.0%')(s.fnr)} of well ${s.titleStr.toLowerCase()} test negative`;
    if (str == 'b')
      html = `${d3.format('.0%')(s.b)} of ${s.titleStr.toLowerCase()} are sick`;
    ttSel.html(html);
  }
  ...
  function makeDrag(str, index) {
    return d3
      ...
      .on('drag', function () {
        ...
        s[str] = d3.clamp(0.15, percent, 0.85);
  ...
  svg
    .append('text')
    .text(s.titleStr)
    .at({textAnchor: 'middle', x: w / 2, y: -8, fontSize: 20});
...
window.basetimer = d3.timer((t) => {
  ...
  if (val < Math.PI * 2) {
    m.b = Math.sin(val + Math.PI / 2) / 4 + 0.4;
  } else if ((Math.PI * 3 < val && val < Math.PI * 5) || true) {
    f.tpr = Math.sin(val + Math.PI / 2) / 4 + 0.4;
  }
  m.update();
});
...
function ttMove(d) {
  ...
  ttSel.style('left', left + 'px').style('top', top + 'px');
}
