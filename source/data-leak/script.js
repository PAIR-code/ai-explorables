...
var color2dark = {
  '#FDE100': d3.color('#FDE100').darker(0.2),
...
var cs = 20;
var cells = d3.cross(d3.range(0, width + cs, cs), d3.range(0, height + cs, cs));
...
function addPlayAnimation(
  ...
  function playAnimation() {
    ...
    var nextPlayers = playerOptions[curPlayerIndex].map((d) =>
      d.map((d) => d * s),
    );

    var interpolates = players.map((d, i) => d3.interpolate(d, nextPlayers[i]));
...
function initField(name) {
  ...
  c.svg = c.svg.append('g').translate([0.5, 0.5]);
  ...
  chalkSel.append('path.white').at({
    d: ['M', Math.round(width / 2), 0, 'V', height].map(roundNum).join(' '),
  });
  chalkSel
    .append('circle.white')
    .at({r: 10 * s})
  ...
  chalkSel.append('path.white').at({
    d: ['M', 0, ((75 - 44) / 2) * s, 'h', 18 * s, 'v', 44 * s, 'H', 0]
      .map(roundNum)
      .join(' '),
  });
  chalkSel.append('path.white').at({
    d: ['M', width, ((75 - 44) / 2) * s, 'h', -18 * s, 'v', 44 * s, 'H', width]
      .map(roundNum)
      .join(' '),
  });
  ...
  if (isRegression) {
    ...
    var regressionSel = c.svg
      .appendMany('path', byColor)
      .at({
        stroke: (d) => color2dark[d.key],
        strokeWidth: 3.5,
        strokeDasharray: '4 4',
      })
      .each(function (d) {
  ...
  var bgPlayerSel = c.svg
    .appendMany('circle.player', players)
  ...
  var textSel = c.svg
    .appendMany(
      'text.chart-title',
      name == 'playerless' ? [players[0], players[20]] : [players[0]],
    )
    .text(
      name == 'regression-leak' || name == 'scatter'
        ? 'New Hire'
        : name == 'playerless'
          ? 'Goalie'
          : '',
  ...
  if (name == 'scatter' || isRegression) {
    ...
    sel
      .insert('div.axis.chart-title', ':first-child')
      .html(
        `
    ...
    chalkSel
      .appendMany('path.white', c.x.ticks(5))
      .at({d: (d) => ['M', Math.round(c.x(d)), '0 V ', c.height].join(' ')});

    chalkSel
      .appendMany('path.white', c.y.ticks(5))
  ...
  function renderCanvas() {
    cells.forEach((d) => {
      players.forEach((p) => {
        ...
        // p.dist = dx*dx + dy*dy
        p.dist = Math.pow(dx * dx + dy * dy, 1.5) + 0.00001;
        p.weight = 1 / p.dist;
