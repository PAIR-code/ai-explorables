...
function drawShapesWithData(classifier) {
  ...
  function drawResults() {
    ...
    function isMatch(rowName, labelName, isRounding) {
      ...
      if (isRounding == true) {
        // No "other" category
        return labelName.includes(toOriginalString(rowName)) ? true : false;
      } else {
        // There is an "other" category, prefixed by "rt_"
        if (labelName == toOriginalString(rowName)) {
          return true;
        } else if (labelName.includes('rt_') && rowName == 'other shapes') {
          return true;
        }
        return false;
      }
    ...
    rowSel
      .appendMany('td', (result) => columns.map((column) => result[column]))
      .text((d) => d)
      .st({
    ...
    header.transition().delay(initialDelay).duration(1000).style('opacity', 1);
    rowSel
      .transition()
      .delay(function (d, i) {
  ...
  function drawDropdown() {
    ...
    ['rounding', 'category'].forEach(function (classifierType) {
      ...
      sel
        .appendMany('option', classifier.options[classifierType])
        .at({
          value: function (d) {
            return d.value;
          },
...
function drawConclusion() {
  ...
  function drawDropdown() {
    ...
    sel.classed('dropdown', true);
    sel
      .appendMany('option', conclusionOptions.category)
      .at({
        value: function (d) {
          return d.value;
        },
