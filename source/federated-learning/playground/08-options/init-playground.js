window.initPlayground = async function(slug){
  ...
  function valueWrap(v, i){ return {v, i} }
  var options = [
    {
  ...
  var optionSel = optionContainerSel
    .appendMany('div.option', options)

  var optionTitleSel = optionSel.append('div.option-title')
    .on('click', option => {
  ...
  optionContainerSel
    ...
    `)
    .select('input').st({fontFamily: 'monospace', borderColor: 'rgba(0,0,0,0)', outline: '1px solid #ccc'})
    .on('input', function(){
  ...
  resetButtonSel.append('span.info-span')
    .datum({info: `Randomize the training data or the model's intial weights.`})
    .st({cursor: 'default'})


  optionContainerSel.append('div')
    .st({position: 'absolute', cursor: 'default'}).translate([585, -150])
    .appendMany('div', [
      {text: `<span class='global-circle'> </span> Global Accuracy`, info: 'The circles indicate the accuracy of the global model the server sees after a merge. This is the metric that matters (in fact, the only metric you can actually compute) in a real federated learning setting.'},
      {text: `<span class='wavy-line'>〰</span> Average Local <br><span class='wavy-line hidden'>〰</span> Accuracy`, info: `We also plot the average of the local accuracies (which a real server would never see, as it doesn't see the local models individually) in order to visualize local federated training dynamics, as in the earlier simulations.`},
    ])
    .st({marginBottom: 20, width: 140, lineHeight: '1em'})
    .append('span').html(d => d.text)
    .parent().append('span.info-span')



  optionContainerSel.selectAll('span.info-span')
    .text(' ⓘ')
  ...
  async function renderOptions(){
    ...
    for (modelSettings of allModelSettings){
      ...
      setTimeout(() => {
        window.graphics.measure(window.graphics.filter(d => d.isFullPlayground)[0])
