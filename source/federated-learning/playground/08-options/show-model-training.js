...
window.showModelTraining = async function(modelSettings, sharedConfig, finishCallback=d=>d){  
  var datatsetSel = sharedConfig.sel.select('.federate-playground-graph').html('')
  ...
  function generateDatasets(){
    ...
    var datasets = d3.range(modelSettings.numClients)
      .map(index => {
        ...
        rv.isDisabled = modelSettings.disabledDatasets && modelSettings.disabledDatasets[index]
        return rv
      })
  ...
  var countSel = sharedConfig.sel.select('.count-display').html('')
  ...
  function resetModels(){
    ...
    datasets.hyperparms = modelSettings.hyperparms || JSON.stringify(modelSettings)
  ...
  function renderModels(isMergeAnimation){
    ...
    countSel
      .text(key => {
        var keyStr = {accuracy: 'Accuracy', local: 'Local Steps', merge: 'Federated Rounds'}[key]
        var numStr = (key == 'accuracy' ? d3.format('.1%') : zeroPad)(datasets.counts[key])
  ...
  if (sharedConfig.mergeModelsButton) sharedConfig.mergeModelsButton.on('click', mergeModelsClick)
