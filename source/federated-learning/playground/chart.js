...
window.chart = (function () {
  ...
  function initLine(sharedConfig) {
    ...
    var showMergeTicks = sharedConfig.allModelSettings.every(
      (d) => d.mergeRate == mergeRate,
    );
    ...
    if (hasLegend) {
      var legendSel = c.svg
        .append('g.legend')
        .translate([c.width + 80, c.height / 2 + (hasSlider ? -0 : 0)]);
      legendSel
        .append('text')
        .st({fontSize: 14})
      ...
      if (sharedConfig.slug == 'outlier') {
        var numOutliers = 5;
        var rs = 17;
        var blockSel = legendSel
          .appendMany('g', d3.range(numOutliers))
          .translate((i) => [(i - numOutliers / 2) * (rs + 2), 5]);
        blockSel.append('rect').at({
          width: rs,
          height: rs,
          fill: (i) => colorScale(i / (numOutliers - 0)),
          xstroke: '#ccc',
        });
        ...
        var n = 120;
        legendSel.appendMany('rect', d3.range(0, 1, 1 / n)).at({
          height: 15,
          y: 4,
          width: 1,
          x: legendScale,
          fill:
            sharedConfig.slug == 'dp' ? (d) => colorScale(d * 5) : colorScale,
        });
      }
    }
    if (sharedConfig.hasDashedLegend) {
      var dashedLegendSel = c.svg
        .append('g.legend')
    ...
    if (hasSlider) {
      ...
      cursorSel
        .append('text')
        .text('▲')
        .at({textAnchor: 'middle', fontSize: 22, dy: '.4em'});
      cursorSel.append('circle').at({r: 16, fillOpacity: 0, strokeWidth: 0});

      legendSel.st({cursor: 'pointer'}).on('click', function () {
      ...
      function renderSlider() {
        cursorSel.translate(
          legendScale(sharedConfig.allModelSettings[0].dpNoise),
          0,
        );
      }
      renderSlider();
    ...
    c.y.domain([0.4, 1]).interpolate(d3.interpolateRound);
    ...
    function fillColor(d, i) {
      var v = hasLegend
        ? d[0]?.hyperparms[0]
        : i / sharedConfig.allModelSettings.length;
      return colorScale(v);
    }
    ...
    function render(log) {
      ...
      if (sharedConfig.lineOnlyGlobal) {
        lineSel2
          ...
          .at({r: 4, fillOpacity: 0.4, strokeWidth: 2});

        // circleSel.at({opacity: 0})
      }
      lineSel.html('').appendMany('path', byKey).at({
        stroke: fillColor,
        strokeWidth: 1.5,
  ...
  function addDatasetHover(selection) {
    selection.classed('hovered', false).on('mouseover', (d) => {
  ...
  function initRescalingLine(sharedConfig) {
    ...
    var stubDatasets = d3
      .range(sharedConfig.allModelSettings[0].numClients)
      .map((index) => ({index, sharedConfig}));

    var datasetLines = c.svg
      .append('g')
    ...
    var lineSel = c.svg
      .append('g')
      .append('path.dataset.dataset-avg')
      .datum(avgPlaceholderData)
      .call(addDatasetHover);
    ...
    setTimeout(() => {
      ...
      d3.select('global-circle')
        .classed('dataset', 1)
        .st({pointerEvents: 'all', cursor: 'default'})
        .datum(mergePlaceholderData)
        .call(addDatasetHover);
    }, 1000);

    var circlesSel = c.svg
      .appendMany('circle.dataset', stubDatasets)
      .at({r: 2, stroke: '#000', fill: '#aaa', strokeWidth: 0.5})
      .call(addDatasetHover);
    ...
    var mergeLineSel = c.svg
      .append('path.dataset.global')
      .datum(mergePlaceholderData)
      .call(addDatasetHover);
    var mergeCircleSel = c.svg
      .append('circle.dataset.global')
      .at({r: 0, stroke: 'cyan', fill: '#000', strokeWidth: 2})
      .datum(mergePlaceholderData)
      .call(addDatasetHover);
    ...
    function render() {
      ...
      var last = _.last(log);
      circlesSel.translate((dataset) => [
        c.x(last.counts.local),
        c.y(last.datasetAccuracy[dataset.index]),
      ]);
      ...
      d3.select('.global-annotation').st({
        opacity: mergePlaceholderData.counts.merge ? 1 : 0,
      });
      if (mergePlaceholderData.counts.merge) {
        ...
        mergeData.push({
          counts: {local: last.counts.local},
          accuracy: mergePlaceholderData.accuracy,
        });
        var line = d3
          .line()
          .x((d) => c.x(d.counts.local))
  ...
  function initHeatmap(dataset, options = {}) {
    ...
    var colors = [color(0.2), color(1 - 0.2)];
    ...
    function drawMergeLines() {
      ...
      if (dataset.sharedConfig.slug == 'top') {
        if (isValidDataset(i - 1) && i % 6 != 0)
          drawMergeLine(['M', 0, c.height / 2, 'h', -35]);
        if (isValidDataset(i - 6))
          drawMergeLine(['M', c.width / 2, 0, 'v', -35]);
      } else {
        if (isValidDataset(i - 1) && i % 8 != 0)
          drawMergeLine(['M', 0, c.height / 2, 'h', -2]);
        if (isValidDataset(i - 8) && i > 7)
    ...
    if (dataset.isGrid) {
      ...
      c.svg
        .selectAll('.y .tick')
        .append('path')
        .at({d: 'M 0 0 H ' + c.width, stroke: '#eee', strokeWidth: 1});
      c.svg.selectAll('.y text').at({x: -3});
      c.svg
        .selectAll('.x .tick')
        .append('path')
        .at({d: 'M 0 0 V -' + c.height, stroke: '#eee', strokeWidth: 1});
    ...
    if (dataset.isGlobal) {
      c.sel
        .datum({index: -2, sharedConfig: dataset.sharedConfig})
        .classed('global', 1);
      c.svg
        .append('text.axis-label')
      ...
      c.accuracySel = c.svg
        ...
        .append('text')
        .at({textAnchor: 'middle', dy: '0.71em'})
        .st({fontSize: 12});
    }
    ...
    function render(isMergeAnimation) {
      if (isMergeAnimation) {
        ...
        d3.select(canvas2)
          ...
          .transition()
          .duration(
            dataset.sharedConfig.mergeAnimation
              ? dataset.sharedConfig.mergeAnimation / 2
              : 300,
          )
          .st({opacity: 0});
