...
window.initPlaygroundTop = async function (slug) {
  ...
  sharedConfig.mergeModelsButton = d3
    .select('merge-models-inline')
    .html('')
    .st({position: 'relative'})
    .append('div.buttons')
    .st({
      display: 'inline-block',
      xfontFamily: 'monospace',
      xfontSize: 13,
      xfontWeight: 200,
    })
    .append('div')
...
window.initPlaygroundStepper = async function (slug) {
  ...
  function onResetModels() {
    var childDataMaps = d3
      .select('.data-map-users')
      .html('')
      .st({margin: '35px 0px'})
      .append('div')
      .st({width: 738, lineHeight: '0px', margin: '0px auto'});

    childDataMaps
      .appendMany('div', sharedConfig.datasets)
    ...
    var allData = {
      data: _.flatten(sharedConfig.datasets.map((d) => d.data)),
      isGrid: 1,
      label: 'all user data',
    };

    allData.sel = d3
      .select('.data-map-all-points')
      .html('')
      .st({margin: '35px 0px'})
      .append('div')
      .st({margin: '0px auto', width: 80});
...
window.initPlaygroundOutlier = async function (slug) {
  ...
  function onDatasetClick(d, resetModelsFn) {
    modelSettings.disabledDatasets[d.index] =
      !modelSettings.disabledDatasets[d.index];
    var pctDisabled = d3.mean(modelSettings.disabledDatasets, (d, i) =>
      d ? NaN : i < modelSettings.numOutliers,
    );
  ...
  d3.select('span.cold').st({
    background: d3.interpolatePuOr(0.1),
    padding: '0px 2px',
    color: '#fff',
  });
  d3.select('span.hot').st({
    background: d3.interpolatePuOr(0.9),
    padding: '0px 2px',
    color: '#fff',
  });
  d3.select('span.outlier').st({
    background: d3.interpolatePuOr(0.1),
    padding: '0px 2px',
    color: '#fff',
  });
...
window.initPlaygroundDp = async function (slug) {
  ...
  modelSettings.hyperparms = [
    modelSettings.dpNoise,
    Math.random(),
    modelSettings.disabledDatasets[0],
  ];
