...
d3.entries(nums).forEach(d => {
  ...
  d.value.color = d.key.length == 1 ? '#000' : d.key.length == 3 ? '#999' : colorScale(d.key)
  // if (d.key == 's') d.value.color = '#999'
})



var sel = d3.select('.secure-aggregation').html('')
  .append('div.block-title').text('Secure Aggregation: How Does It Work?').st({marginBottom: 10})
  .parent().append('div.container')

sel.appendMany('div', d3.range(3))
  .each(drawDiagram)
...
!(function(){
  ...
  var numSel = sel.selectAll('.num')
    .call(drag)
    // .st({cursor: 'ew-resize', pointerEvents: d => d.n.key.includes('s') ? 'none' : ''})
    .st({cursor: 'ew-resize', pointerEvents: d => d.n.key.includes('s') || d.n.key.length== 1 ? 'none' : ''})
...
function drawDiagram(diagramIndex){
  ...
  var peopleSel = divSel.appendMany('div', people)
    .st({position: 'absolute', width: blockWidth})
    .translate(d => d.pos)

  var nameSel = peopleSel.append('div')
    .st({textAlign: 'center', position: 'absolute', width: blockWidth, top: d => d.personIndex == 0 ? -20 : 17 + 10})
    .text(d => d.name)

  var blockSel = peopleSel.append('div.num-container')

  var numSel = blockSel.appendMany('span.num', d => d.activeNums)
    .st({color: '#fff', background: d => d.n.color})

  numSel.filter(d => d.n.key == 's')
    .st({color: '#000', background: '#fff', outline: '1px solid #000'})

  var peopleSvgSel = c.svg.appendMany('g', people)
  ...
  if (diagramIndex == 1){
    ['ab', 'bc', 'ca'].forEach((pairStr, pairIndex) => {
      ...
      var pairNodes = []
      numSel.filter(d => d.n.key == pairStr)
        .each(function(d){ pairNodes.push(this)})

      if (d3.select(pairNodes[0]).datum().personIndex < d3.select(pairNodes[1]).datum().personIndex)pairNodes.reverse()
      ...
      if (pairIndex == 1){
        ...
        midToMid(pairNodes[0], pairNodes[1], {stroke, x0: pairIndex ? -2 : -2, x1:  5, })
  ...
  peopleSel.each(function(d){
    ...
    var rectSel = c.svg.append('rect').lower()
  ...
  function midToMid(a, b, options={}){
    ...
    var [x0, x1] = [aBox.x + aBox.width/2,  bBox.x + bBox.height/2].map(d => d - pBox.x)
    ...
    c.rootsvg.append('path.dotted-path')
      .at({
        d: `M ${[x0, y0]} L ${[x1, y1]}`,
  ...
  function topMidToMid(a, b, offsets={}){
    ...
    var [x0, x1] = [aBox.x + aBox.width/2,  bBox.x + bBox.height/2].map(d => d - pBox.x)
    var [y0, y1] = [aBox.y + aBox.height*0, bBox.y + bBox.height*0].map(d => d - pBox.y)
  ...
  var captionSel = c.sel.parent()
    .st({width: c.totalWidth})
    .append('div.caption').html(captions[diagramIndex])
  ...
  if (diagramIndex == 0){
    ...
    captionSel.selectAll('total-container')
      .append('total')
