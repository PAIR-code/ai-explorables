...
people.forEach((person) => {
  var prevTemplates = [];
  person.messages = d3.range(12).map((d) => {
    var noun = _.sample(nouns);
    var isSpam = noun == spamNoun;
    if (random() < 0.1) isSpam = !isSpam;

    var nonDupNormalTemplates = normalTemplates.filter((d) =>
      prevTemplates.every((e) => e != d),
    );
...
function drawCentral() {
  var c = d3.conventions({
    sel: d3
      .select('.central-spam-model')
      .html('')
      .classed('spam-model', 1)
      .st({margin: '0px auto'}),
    width: 605,
    layers: 'sd',
  ...
  divSel
    .append('div.caption')
    .html(
      `With <b>centralized training</b>, each user's data is uploaded to a server where a single model is trained.`,
    )
    .st({maxWidth: 320});
}
drawCentral();
...
function drawLocal() {
  ...
  peopleSel.each(function (d, i) {
    midRightToMidLeft(
      d3.select(this),
      d3.select(peopleModelSel._groups[0][i]),
      c,
      0,
      2,
      0,
  ...
  divSel
    .append('div.caption')
    .html(
      'Each user <b>locally trains</b> a spam model independently — no information is shared with a server.',
    )
    .st({maxWidth: 350});
}
drawLocal();
...
function drawFederated() {
  ...
  divSel
    .append('div.caption')
    .html(
      '<b>Naively combining models</b> across users produces a shared model but could compromise user privacy.',
    )
    .st({maxWidth: 350});
}
drawFederated();
...
function drawSecureFederated() {
  var c = d3.conventions({
    sel: d3
      .select('.secure-federated-spam-model')
      .html('')
      .classed('spam-model', 1),
    width: 699,
    layers: 'sd',
  ...
  divSel
    .append('div.caption')
    .html(
      '<b>Federated learning</b> — shown here with secure aggegation — produces a shared model while protecting user privacy.',
    )
    .st({maxWidth: 410});
}
drawSecureFederated();
...
function addChatMsgHover(selection, parentSel) {
  selection
    ...
    .on('mouseout', (d) => {
      d3.selectAll('.chat-msg-hover').classed('hovered', 0);
    });
}
...
function drawPeopleSel(allMessages, divSel) {
  ...
  var messageSel = peopleSel
    .append('div.chat-message-container.no-scroll-bar')
...
function drawPeopleModelSel(allMessages, divSel) {
  ...
  peopleModelSel.each(function (person) {
    drawModelSel(
      allMessages.filter((d) => d.person == person),
      d3.select(this),
      person.name + "'s ",
    );
  });
...
function drawServerAggregationSel(divSel) {
  var serverAggregationSel = divSel
    .append('div')
    .st({marginLeft: 50})
    .st({paddingBottom: 10, marginBottom: 10, paddingLeft: '2px', height: 145});
}
...
function drawServerSel(allMessages, divSel, isFederated, isSecure) {
  var serverSel = divSel
    .append('div.server-container')
    .st({width: isFederated ? 200 : ''});

  if (isFederated) {
    var messageByNounPerson = _.sortBy(
      d3.nestBy(allMessages, (d) => [d.noun, d.person.name]),
      (d) => d.key,
    );
    messageByNounPerson.forEach(
      (d) => (d.percentSpam = d3.mean(d, (e) => e.isSpam)),
    );

    if (isSecure) {
      var messageByNoun = _.sortBy(
        d3.nestBy(messageByNounPerson, (d) => d.key.split(',')[0]),
        (d) => d.noun,
      );
      messageByNoun.forEach(
        (d) => (d.percentSpam = d3.mean(d, (e) => e.percentSpam)),
      );

      var aggSel = serverSel.append('div.aggregation-container');
      aggSel.append('div.block-title').text('Secure Aggregation');
      var aggBox = aggSel
        .append('div.aggregation-box')
        .html(`<i class='material-icons'>lock</i>`);
      serverSel.append('div.block-title').text('Central Server');
      var tableSel = serverSel.append(
        'div.data-table.aggregated.no-scroll-bar',
      );
      ...
      renderSpam.fns.push(() => {
        ...
        messageByNoun.forEach(
          (d) => (d.percentSpam = d3.mean(d, (d) => d.percentSpam)),
        );
        rowSel.html(
          (d) => `
      ...
      var rowSel = tableSel.appendMany(
        'div.chat-msg-hover',
        messageByNounPerson,
      );
      renderSpam.fns.push(() => {
    ...
    var tableSel = serverSel.append('div.data-table.no-scroll-bar');
    var rowSel = tableSel
      .appendMany('div.chat-msg-hover', _.shuffle(allMessages))
      .on('click', (d) => {
        d.isSpam = !d.isSpam;
        renderSpam();
      })
...
function drawModelSel(allMessages, parentSel, name = '', isFederated) {
  var messageByNoun = _.sortBy(
    d3.nestBy(allMessages, (d) => d.noun),
    (d) => d.key,
  );
  ...
  var wordWeightSel = modelSel
    .append('div.model-table.no-scroll-bar')
  ...
  renderSpam.fns.push(() => {
    ...
    wordWeightSel
      .append('div')
      .text((d) => d.key)
      .st({width: 40, position: 'relative', top: 5});
    wordWeightSel
      .append('div')
      .st({
        height: 17,
      ...
      .st({
        top: 2,
        position: 'absolute',
        left: (d) => `calc(${d.percentSpam * 100}% + 2px)`,
      })
      .filter((d) => d.percentSpam > 0.5)
      .st({
        left: '',
        right: (d) => 90 * (1 - d.percentSpam) + 2,
        color: '#fff',
      });
  });
...
function midRightToMidLeft(
  ...
  c.svg.append('circle').at({r: 4, cx: x1, cy: y1, fill: '#888'});
}
...
function midBotToMidLeft(a, b, c, strokeDasharray) {
  ...
  var [y0, y1] = [aBox.y + aBox.height, bBox.y + bBox.height / 2].map(
    (d) => d - pBox.y,
  );
...
function midBotToMidTop(a, b, c, strokeDasharray) {
  ...
  var [x0, x1] = [
    aBox.x + (7 * aBox.width) / 8,
    bBox.x + (7 * bBox.width) / 8,
  ].map((d) => d - pBox.x);
