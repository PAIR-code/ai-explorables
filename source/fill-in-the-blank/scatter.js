...
window.initScatter = function(c){
  ...
  var centerPathSel = c.svg.selectAppend('path.center')
    .st({pointerEvents: 'none', strokeWidth: .3, stroke: '#ccc'})
  ...
  rv.draw = function(c, data, isxy){
    ...
    var tinyTextSel = svg.selectAll('text.tiny')
      .data(data.filter(d => d.show), d => d.word)
    ...
    tinyTextSel.enter().append('text.tiny')
      .text(d => d.prettyWord)
      .at({
    ...
    c.svg.transition().duration(duration)
    ...
    centerPathSel
      ...
      .at({d: isxy ? 
        ...
        ['M', 0, c.y(0) + .5, 'L', c.width, c.y(0) + .5].join(' ')
      })
  ...
  function addHover(){
    ...
    svg
      .on('mousemove', function(){
        ...
        var match = _.minBy(rv.data.filter(d => d.isVisible), d => {
          var dx = x - d.x
          var dy = y - d.y
