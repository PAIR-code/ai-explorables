...
window.initCircleWeightsVis = async function ({
  ...
  var pointData = d3
    .range(Math.min(150, type == 'embed' ? state.n_tokens : state.n_tokens * 2))
    .map((i) => ({i}));
  ...
  state.renderAll.input.fns.push(() => {
    ...
    if (type == 'embed') {
      ...
      pointSel
        .selectAll('text')
        .text((d) =>
          d.i == target || d.i % 2 == 1 ? d.i % state.n_tokens : '',
        );
      pointSel.classed('active', (d) => d.i == target);
    }

    abTextSel.translate((d) => c.xy(mul(state.embed[state[d]], 1.2)));
