...
window.initProjVis = async function ({type, state, sel, title, caption}) {
  ...
  sel
    .select('.directions-caption')
    .appendMany('span', state.neurons)
    .st({
  ...
  c.svg.append('text').text(title).at({y: -5, fontSize: 14, fontWeight: 600});
  ...
  c.svg
    .append('circle')
    .translate(c.xy([0, 0]))
    .at({r: c.x(max * 0.7) / 2, stroke: '#ccc', fill: 'none'});

  var pointData = d3.range(Math.min(state.n_tokens, 100)).map((i) => ({i}));

  var lineSel = c.svg
    .appendMany('path', pointData)
    .st({opacity: isIn ? 0 : 0.2});

  if (!isIn) {
    c.svg
      .appendMany('path', state.neurons)
      .at({
        stroke: (d) => d.color,
        strokeDasharray: (d) => (d.isDash ? '2 1' : ''),
        strokeWidth: 1,
      })
      .at({d: (d) => 'M' + c.xy([0, 0]) + 'L' + c.xy(mul(d[type], 0.93))})
      .translate((d) => {
  ...
  c.svg
    .appendMany('path', state.neurons)
    .at({
      stroke: (d) => (isIn ? d.color : 'rgba(0,0,0,0)'),
      strokeDasharray: (d) => (d.isDash ? '2 1' : ''),
      strokeWidth: 1,
    })
    .at({d: (d) => 'M' + c.xy([0, 0]) + 'L' + c.xy(mul(d[type], 0.93))})
  ...
  var projSel = c.svg.appendMany('path', state.neurons).at({
    stroke: (d) => d.color,
    strokeDasharray: (d) => (d.isDash ? '2 1' : ''),
    strokeWidth: 4,
  });
  ...
  var textSel = pointSel
    .append('text')
    .text((d) => d.i % state.n_tokens)
    .at({textAnchor: 'middle', dy: '.33em', fontSize: 10});

  pointSel.append('circle').at({r: 3});

  var connectionPathSel = c.svg
    .append('path')
    .at({stroke: '#000', strokeDasharray: '2 2'});
  var meanPathSel = c.svg.append('path').at({stroke: '#000'});
  var meanCircleSel = c.svg
    .append('circle')
    .at({r: 4, fill: '#F1F3F4', stroke: '#000'});
  var correctCircleSel = c.svg
    .append('circle')
    .at({r: 4, fill: '#F1F3F4', stroke: '#f0f'})
    .st({opacity: isIn ? 0 : 1});
  ...
  state.renderAll.input.fns.push(() => {
    ...
    if (isIn) {
      pointSel.st({
        opacity: (d) => (d.i == state.a || d.i == state.b ? 1 : 0.2),
      });
      connectionPathSel.at({d: 'M' + c.xy(aPos) + 'L' + c.xy(bPos)});
    } else {
      pointSel.st({opacity: 0.2});
    ...
    lineSel.st({
      stroke: (d) => (d.i == state.correct ? '#f0f' : 'rgba(0,0,0,0)'),
    });
