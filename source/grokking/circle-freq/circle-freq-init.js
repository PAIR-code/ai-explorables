...
window.circleFreqInit = () => {
  ...
  function drawProjectionCircle(sel) {
    ...
    centerSel.append('circle').at({r, stroke: '#999', fill: 'none'});
    ...
    var aCircleSel = centerSel
      .append('circle')
      .at({r: 5, stroke: 'steelblue', fill: 'none', strokeWidth: 2});

    var bCircleSel = centerSel
      .append('circle')
      .at({r: 5, stroke: 'orange', fill: 'none', strokeWidth: 2});

    var aLineSel = centerSel
      .append('path')
      .at({stroke: 'steelblue', fill: 'none'});
    var bLineSel = centerSel
      .append('path')
      .at({stroke: 'orange', fill: 'none'});
    ...
    renderAll.fns.push(() => {
      ...
      function calcPos(x) {
        var spiralSpacing = 0.5; // the space between successive turns of the spiral
      ...
      aPoints.forEach((d) => {
        var [x, y] = calcPos(d.v * a);
        d.x = x;
        d.y = y;
      });
      bPoints.forEach((d) => {
        var [x, y] = calcPos(a + d.v * b);
        d.x = x;
        d.y = y;
      });
