...
window.initFiveNeurons = async function () {
  ...
  state.renderAll.step.fns.push(() => {
    ...
    for (var i = 0; i < shape[1]; i++) {
      for (var j = 0; j < shape[2]; j++) {
        ...
        if (i == 0) {
          state.model.w_inproj[j] = [
            state.w_inproj.data[index],
            state.w_inproj.data[index + 5],
          ];
          state.model.w_outproj[j] = [
            state.w_outproj.data[index],
            state.w_outproj.data[index + 5],
          ];
        }
      }
    }
  ...
  var annotations = [
    {
      ...
      'minWidth': 850,
      'html':
        "<b style='color:var(--color-train)'>Train</b> and <b style='color:var(--color-test)'>test</b> accuracy don't diverage; the model is too small to memorize",
      'st': {
        'top': 120,
        'left': 390,
        'width': 200,
  ...
  util.addAria([
    {
      selector: '.five-neuron-accuracy',
      label: `Accuracy over training. accuracy don't diverage; the model is too small to memorize `,
    },
    {selector: '.five-neuron-embed', label: `W_in-proj and W_out-proj heatmap`},
    {
      selector: '.five-neuron-circle',
      label: `Neurons W_in-proj and W_out-proj plotted on a circle`,
    },
    {
      selector: '.five-neuron-circle-2',
      label: `W_in-proj and W_out-proj plotted on a circle with connecting lines. They form a star on W_out-proj!`,
    },
    {
      selector: '.mod-bot-seeds',
      label: `Small multiple loss charts showing many training runs.`,
    },
  ]);
};
