...
window.initActivationVis = async function (state) {
  ...
  function initHidden(d) {
    ...
    var hoverTickA = c.svg
      ...
      .raise()
      .classed('hover-tick', 1);

    var hoverTickB = c.svg
      .select('.y .tick')
      .select(function () {
        return this.parentNode.insertBefore(
          this.cloneNode(true),
  ...
  function initOut(d) {
    ...
    var valTextSel = c.svg.append('text').at({y: -2, fontSize: 12});

    var lineSel = c.svg
      .appendMany('path', d3.range(state.n_tokens))
  ...
  function initSum() {
    ...
    c.svg.append('text').text('Logits Sum').at({y: -2, fontSize: 12});
  ...
  function render() {
    ...
    hiddenData.forEach((hidden) => {
      ...
      hiddenData.activations[i].forEach((row, i) => {
        row.forEach((v, j) => {
          ...
          ctx.rect(c.x(i), c.y(j), s - 0.1, s - 0.1);
          ctx.fill();
        });
      });
    });
    ...
    hiddenData.maxVal =
      d3.max(hiddenData.activations.flat(2)) *
      d3.max(hiddenData.outW_embedW.flat(2));
    hiddenData.forEach((hidden) => {
    ...
    !(function () {
      ...
      hiddenData.sum.hoverTick = c.svg
        .select('.x .tick')
        .select(function () {
          return this.parentNode.insertBefore(
            this.cloneNode(true),
  ...
  renderAll.input.fns.push(() => {
    ...
    hiddenData.forEach(({hidden}) => {
      ...
      hoverSel.translate((d) => [c.x(state.a), c.y(state.b)]);
    ...
    !(function () {
      ...
      var vals = d3.range(state.n_tokens).map((i) => {
        return d3.sum(
          d3.range(state.hidden_size).map((j) => hiddenData[j].out.vals[i]),
        );
      });
