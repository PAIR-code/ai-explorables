...
window.initCircleInputVis = async function ({type, state}) {
  ...
  function initCircle(hiddenIndex) {
    ...
    c.svg
      .append('text')
      .text(type.replace('hidden', 'input') + ' ' + hiddenIndex)
      .at({y: -5, fontSize: 12});

    var valTextSel = c.svg
      .append('text')
      .at({y: -2, fontSize: 12, x: c.width, textAnchor: 'end'});

    var lineSel = c.svg.append('path').at({stroke: '#000'});
    var lineNegSel = c.svg
      .append('path')
      .at({stroke: '#000', strokeDasharray: '2 2'});

    var thickLineSel = c.svg
      .append('path')
      .at({stroke: '#64FFDA', opacity: 1, strokeWidth: 5});
    var circleSel = c.svg
      .appendMany(
        'circle',
        isOutEmbed ? d3.range(state.hidden_size) : [hiddenIndex],
      )
      .at({r: 7, fill: '#64FFDA', stroke: '#03c697', strokeWidth: 2});

    var circleNumSel = c.svg
      .appendMany(
        'text',
        isOutEmbed ? d3.range(state.hidden_size) : [hiddenIndex],
      )
      ...
      .style(
        'text-shadow',
        '0 1px 0 #64FFDA, 1px 0 0 #64FFDA, 0 -1px 0 #64FFDA, -1px 0 0 #64FFDA',
      );

    state.renderAll.model.fns.push(render);

    var centerSel = c.svg.append('g').translate([c.width / 2, c.height / 2]);

    var aLineSel = centerSel.append('path').at({
      strokeWidth: 2,
      opacity: 1,
      stroke: util.colors.aInput,
      fill: 'none',
    });
    var bLineSel = centerSel.append('path').at({
      strokeWidth: 2,
      opacity: 1,
      stroke: util.colors.bInput,
      fill: 'none',
    });

    var aPoints = d3.range(0, 1 + 0.001, 0.001).map((v) => ({v, x: 0, y: 0}));
    var bPoints = d3.range(0, 1 + 0.001, 0.001).map((v) => ({v, x: 0, y: 0}));

    var aCircleSel = c.svg
      .append('circle')
      .at({r: 5, stroke: util.colors.aInput, fill: 'none', strokeWidth: 3});
    var bCircleSel = c.svg
      .append('circle')
      .at({r: 5, stroke: util.colors.bInput, fill: 'none', strokeWidth: 3});
    ...
    state.renderAll.input.fns.push(() => {
      ...
      if (ogType == 'outW') {
        ...
        circleNumSel
          .translate((i) => state.temp_out_w[i])
          .st({opacity: (i) => (state.temp_out_w[i].hiddenVal > 0 ? 1 : 0)});

        var mean = [
          d3.mean(state.temp_out_w, (d) => d[0]),
          d3.mean(state.temp_out_w, (d) => d[1]),
        ];
        var sum = [
    ...
    function render() {
      ...
      c.r = c.x(max * 0.7) / (isOut ? 2 : 2);

      c.svg
        .append('circle')
        .translate([c.width / 2, c.height / 2])
        .at({r: c.r, stroke: '#ccc', fill: 'none'});
