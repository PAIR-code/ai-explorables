...
window.initEmbedCircleVis = async function ({type, state, sx = 20, sy = 20}) {
  ...
  c.svg
    .select('.x .axis-label')
    .parent()
    .translate([c.width / 2 - sx / 2, 30]);
  c.svg
    .select('.y .axis-label')
    .parent()
    .translate([-25, c.height / 2 - (isEmbed ? -sy / 2 : sy / 2)]);

  c.svg
    .append('rect')
    .at({width: c.width + 0.6, height: c.height + 0.6, x: -0.5, y: -0.5});

  var typeLabelSel = c.svg
    .append('text.chart-title')
    .at({y: -5, fontSize: 12, x: c.width / 2, textAnchor: 'middle'})
    .text(
      'W_' +
        ppType
  ...
  function render() {
    ...
    for (var i = 0; i < shape[0]; i++) {
      for (var j = 0; j < shape[1]; j++) {
        ...
        ctx.rect(i * sx, j * sy, sx - 0.5, sy - 0.5);
        ctx.fill();
      }
    }
  }
  ...
  function drawLegend(isUpdate) {
    ...
    var legendSel = c.svg
      ...
      .translate([c.width + 30, -c.height / 2]);

    legendSel.selectAll('text').remove();
    legendSel
      .appendMany('text', y.ticks(2))
      .text(d3.format('+'))
