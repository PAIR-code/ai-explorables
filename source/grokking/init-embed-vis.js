...
window.initEmbedVis = async function ({
  state,
  sel,
  type,
  sx = 3,
  ...
  var stepLabelSel = c.svg.append('text').at({
    textAnchor: 'end',
    x: c.width,
    y: -5,
    fontSize: 12,
    opacity: isTiny || isNoStep ? 0 : 1,
  });
  ...
  if (isCos) {
    c.svg
      .append('rect')
      .at({width: c.width + 0.6, height: c.height + 0.6, x: -0.5, y: -0.5});
  } else {
  ...
  state.renderAll.step.fns.push(() => {
    stepLabelSel.text(
      (c.width > 200 ? 'Training Step ' : 'Step ') +
        d3.format('05,')(state.stepIndex * state.hyper.save_every),
    );
  ...
  var topSvg = c.layers[2].on('mousemove', function () {
    var y = d3.mouse(this)[1];
    state.dim = Math.floor(y / sy);
    state.renderAll.dim?.();
  });
  topSvg.on('mouseleave', () => {
    state.dim = -1;
    state.renderAll.dim?.();
  });
  ...
  var holeSel = topSvg
    ...
    .parent()
    .append('rect')
    .at({width: c.width, height: c.height, y: 0});

  topSvg.append('rect').at({
    width: c.width,
    height: c.height,
    fillOpacity: 0.5,
    mask: `url(#${uuid})`,
  ...
  state.renderAll.dim?.fns.push(() => {
    if (state.dim >= 0) {
      ...
      hoverTick.st({opacity: 1}).translate(sy * state.dim + 0.5, 1);
  ...
  function drawLegend(isUpdate) {
    ...
    var legendSel = topSvg
      .selectAppend('g.axis.legend')
      .translate(c.width + (type == 'out_w' ? 80 : isCos ? 40 : 20), 0);

    legendSel.selectAll('text').remove();
    legendSel
      .appendMany('text', y.ticks(5))
      .text(d3.format('+'))
      .at({x: 35, dy: '.33em', y: y, textAnchor: 'end'});

    if (isUpdate) return;
    legendSel.appendMany('rect', d3.range(nTicks).map(y.invert)).at({
      width: 20,
      height: 1,
      y: (d, i) => i,
      fill: (d) => color(d),
