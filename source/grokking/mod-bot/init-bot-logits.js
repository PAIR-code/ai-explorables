...
window.initModBotLogits = async function ({state, sel, stepsKey}) {
  ...
  sel
    .appendMany('div.all-logits', [0])
    .st({display: 'block', textAlign: 'center'})
    .append('div')
    .each(drawFreqChart);
  ...
  function updateTfWeights() {
    ...
    state.tfOut = util
      .npy2tfSlice(state.out_embed_t_w, state.stepIndex)
      .transpose();
  }
