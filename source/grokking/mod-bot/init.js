...
window.initModBot = async function () {
  ...
  d3.selectAll('.small-multiple-seed').classed(
    'active',
    (d) => d.hyper.slug == state.slug,
  );
  ...
  if (state.isSorted) {
    ...
    state.dft_hidden_embed_w = await util.getFile(
      state.modelPath + '/dft_hidden_embed_w_sorted.npy',
    );
    state.dft_out_embed_w = await util.getFile(
      state.modelPath + '/dft_out_embed_w_sorted.npy',
    );
  } else {
    state.hidden_embed_w = await util.getFile(
    ...
    state.dft_hidden_embed_w = await util.getFile(
      state.modelPath + '/dft_hidden_embed_w.npy',
    );
    state.dft_out_embed_w = await util.getFile(
      state.modelPath + '/dft_out_embed_w.npy',
    );
  }
  ...
  function calcByFreq(state) {
    ...
    var byFreq = _.sortBy(
      d3.nestBy(state.dft_max, (d) => d.freq),
      (d) => +d.key,
    );
    var prevFreqCount = 0;
    byFreq.forEach((freq) => {
      freq.forEach((hiddenDim) => {
        hiddenDim.stepsHidden = d3.range(shape[0]).map((stepIndex) => {
          ...
          var cos =
            dft_hidden_embed_w.data[
              offset + shape[2] * (hiddenDim.freq * 2 + 1)
            ];
          var sin =
            dft_hidden_embed_w.data[
              offset + shape[2] * (hiddenDim.freq * 2 + 2)
            ];
        ...
        hiddenDim.stepsOut = d3.range(shape[0]).map((stepIndex) => {
          var offset = shape[1] * shape[2] * stepIndex + hiddenDim.index;
          var cos =
            dft_out_embed_w.data[offset + shape[2] * (hiddenDim.freq * 2 + 1)];
          var sin =
            dft_out_embed_w.data[offset + shape[2] * (hiddenDim.freq * 2 + 2)];
          var norm = Math.sqrt(cos * cos + sin * sin);
          return {cos, sin, norm};
        });
  ...
  util.addAria([
    ...
    {
      selector: '.mod-bot-waves',
      label: `W_input and W_output plotted as a line chart`,
    },
    {
      selector: '.mod-bot-dft',
      label: `discrete Fourier transform of W_input and W_output as a heatmap`,
    },
    {
      selector: '.mod-bot-freqs-hidden',
      label: `W_input dft neurons grouped by frequency plotted on a circle`,
    },
    {
      selector: '.mod-bot-freqs-hidden',
      label: `W_output dft neurons plotted on a circle. The connections make a star!`,
    },
    {
      selector: '.mod-bot-sliders',
      label: `Bar chart showing model logits. They form a wave at the end of training.`,
    },
  ]);
};
window.initModBot();
