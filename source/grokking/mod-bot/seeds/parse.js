...
var sweeps = ['mlp_modular_addition_sweep'];

sweeps.forEach((sweep) => {
  var dir =
    __dirname +
    '/../../../../../tiny-transformers/local-data/mlp_modular/' +
    sweep;
  ...
  jp.nestBy(paths, (d) => d.split(`mlp_modular/${sweep}/`)[1].split('/')[0])
    ...
    .map((d, modelIndex) => {
      ...
      var minMax = 10;
      var byFreq = jp.nestBy(
        dft_max.filter((d) => d.max_val > minMax),
        (d) => d.freq,
      );
      ...
      hypers.push(rv);
      allMetrics.push(
        metrics.map(({step, train_loss, eval_loss}) => ({
          modelIndex,
          step,
          train_loss,
          eval_loss,
  ...
  allMetrics = allMetrics.flat().filter((d) => d.step % 1000 == 0);
  ...
  io.writeDataSync(
    __dirname + '/data__allmetrics_' + sweep + '.csv',
    allMetrics,
  );
});
