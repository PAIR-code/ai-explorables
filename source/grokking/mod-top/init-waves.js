...
window.initModTopWaves = async function ({
  state,
  sel,
  type,
  ...
  var freqSel = sel
    .html('')
    .st({paddingTop: 10, paddingBottom: 10})
    .appendMany('div', byFreq)
    .st({width: 400})
    .each(drawFreqChart);
  ...
  function drawFreqChart(freq, chartIndex) {
    ...
    var stepLabelSel = c.svg.append('text.chart-title').at({
      textAnchor: 'end',
      x: c.width,
      y: -5,
      fontSize: 12,
      opacity: chartIndex ? 0 : 1,
    });
    ...
    var lineSel = c.svg
      .appendMany('path.wave-path', freq)
      .at({stroke: '#000', fill: 'none', opacity: 0.4})
      .on('mouseover', (d) => {
        state.dim = d.index;
        state.renderAll.dim();
      });
    ...
    state.renderAll.step.fns.push(() => {
      stepLabelSel.text(
        (c.width > 200 ? 'Training Step ' : 'Step ') +
          d3.format('06,')(state.stepIndex * state.hyper.save_every),
      );
