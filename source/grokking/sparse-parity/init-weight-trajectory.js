...
window.initParityWeightsTrajectory = async function ({state, sel, type}) {
  ...
  c.svg
    ...
    .at({y: -5, fontSize: 12, xz: c.width / 2, textAnchorz: 'middle'})
  ...
  c.svg.appendMany('path', weightArray).at({
    stroke: (d) =>
      d.isTopK ? util.colors.highlight : d.isBias ? 'pink' : '#000',
    opacity: (d) => (d.isTopK || d.isBias ? 1 : 0.1),
    fill: 'none',
  ...
  c.svg
    ...
    .on('mousemove', function () {
      state.stepIndex = Math.floor(
        c.x.invert(d3.mouse(this)[0]) / state.hyper.save_every,
      );
