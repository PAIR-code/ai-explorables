...
window.modSweepState = window.modSweepStatex || {
  ...
  hyper_sweep: {
    'seed': d3.range(9),
    'weight_decay': [1e-2, 3e-2, 1e-1, 3e-1, 1],
...
window.initModSweep = async function () {
  ...
  state.isHoveredFn = (d) => {
    var h = state.hovered;
    return (
      d[state.key_row] == h[state.key_row] &&
  ...
  state.modelsL1 = await util.getFile(
    '/mlp_modular/02-sweep-architecture/data__hypers_xm_gpu_full_l1_architecture.csv',
  );
  state.modelsL2 = await util.getFile(
    '/mlp_modular/02-sweep-architecture/data__hypers_xm_gpu_full_l2_architecture_v2.csv',
  );
  state.sharedHyper = await util.getFile(
    '/mlp_modular/02-sweep-architecture/hyper_shared.json',
  );
  ...
  state.renderAll.type.fns.push(() => {
    ...
    state.sweepSlug = isL1
      ? 'xm_gpu_full_l1_architecture'
      : 'xm_gpu_full_l2_architecture_v2';

    // remove previous listeners
    state.renderAll.hover.fns = state.renderAll.hover.fns.filter(
      (d) => !d.isRight,
    );
    state.renderAll.color.fns = state.renderAll.color.fns.filter(
      (d) => !d.isRight,
    );
