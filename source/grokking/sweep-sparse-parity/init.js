...
d3.loadData(
  `sweep-sparse-parity/data__hypers_${spSweepState.sweepSlug}.csv`,
  'sweep-sparse-parity/hyper_shared.json',
  (err, res) => {
    // console.clear()
  ...
  `);
    ...
    function drawLineCharts({state, sel}) {
      ...
      function drawChart(chartIndex) {
        ...
        c.y = d3.scaleLog().domain([1, 1e-8]).range([0, c.height]);
    ...
    function drawGrid({state, sel}) {
      sel
        ...
        .appendMany(
          'div.lr-row',
          d3.nestBy(
            _.sortBy(data.models, (d) => +d[state.key_row]),
            (d) => d[state.key_row],
          ),
        )
      ...
      function drawGridChart(models, i) {
        ...
        c.svg
          .append('text.axis-label')
          .text(
            util.keyFmt(state.key_col) +
              ': ' +
              d3.format('.2f')(models[0][state.key_col]),
          )
