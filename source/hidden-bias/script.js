...
var axis2Sel = c.svg
  .append('g.axis')
  .append('line')
  .translate(Math.round(c.y(2)) + 0.5, 1)
...
var students = d3.range(150).map((d, index) => {
  ...
  var hsGPA = collegeGPA + d3.randomNormal.source(seed)(meanGPADiff, 0.8)();
  ...
  var maleOffset = rand() * (isMale ? 1 : -1) * 0.6;

  // if (index == 47) name = 'Mia'
  // if (index == 82) name = 'Mason'

  var compGPA0 = lerp(hsGPAadjusted, collegeGPA, rand() * 0.7) + maleOffset;
  ...
  var isLowIncome = rand() < 0.5;

  var inteviewGPA = collegeGPA + d3.randomNormal.source(seed)(0, 0.15)();
  var inteviewGPAbias = inteviewGPA + rand() * (isLowIncome ? -1 : 1) * 0.5;
...
!(function () {
  ...
  var htmlAnnoSel = divSel
    .appendMany(
      'div.annotation',
      annotations.filter((d) => d.html),
    )
  ...
  window.annotationSel = swoopySel
    .selectAll('g')
...
var innerCircleSel = circleSel.append('circle').at({
  r: 5,
  fill: '#eee',
  stroke: '#aaa',
});
...
var longLabel =
  'high school GPA, essay, clubs, zip code, teacher recommendations, sports, AP scores, demonstrated interest, gender, SAT scores, interviews, portfolio, race, work experience';
...
var slides = [
...

  {
    yKey: 'compGPA0',
    yLabel:
      'high school GPA, essay, clubs, zip code'.replace(
        'essay',
        '<span class="highlight blue">essay',
      ) + '</span>',
  },

  {
    yKey: 'compGPA1',
    yLabel:
      longLabel.replace('teacher', '<span class="highlight blue">teacher') +
      '</span>',
    circleFill: 'grey',
    circleFillDelay: (d) => 0,
    textFill: '#eee',
...

  {
    textFill: '#eee',
    yLabel:
      'Alumni interview, household income'.replace(
        'household',
        '<span class="highlight blue">household',
      ) + '</span>',
    yKey: 'inteviewGPA',
    text2Opacity: 1,
  },
...
slides.forEach((d) => {
  ...
  if (d.yLabel[0] != '<')
    d.yLabel =
      '<b>Predicted College GPA</b> using ' +
      d.yLabel.replace('School', 'school');
});
...
function updateSlide(i) {
  ...
  axis2Sel.transition().st({opacity: i == 5 ? 1 : 0});
  ...
  annotationSel.transition().st({opacity: (d) => (i == d.slide ? 1 : 0)});
