window.initColumns = function (id, metrics, measures) {
  ...
  var colSel = c.svg
    .appendMany('g', measures)
    .translate((d, i) => [0.5 + i * (colW + colWpad) + offsetW, 0.5]);

  colSel
    .append('text')
    .text((d) => d.ranking_display_text)
    .at({y: -20, textAnchor: 'middle', x: colW / 2, fontWeight: 600});

  var rowSel = colSel
    .appendMany('g.row', sets)
    .translate((d) => d.i * (colH + colHpad), 1);

  var colMean = colSel.filter((d, i) => i === 0);
  var colMin = colSel.filter((d, i) => i === 1);
  var scoreLabelsMean = colMean
    .selectAll('.row')
    .append('text')
    .at({x: -5, y: 15, textAnchor: 'end'})
    .st({fontSize: '13px', opacity: 0.7});
  var scoreLabelsMin = colMin
    .selectAll('.row')
    .append('text')
  ...
  var l0Sel = c.svg
    .appendMany('path.set', sets)
    .classed('set1', true)
    .translate((d) => [colW + offsetW, s / 2 + 0.5]);

  drawRow(rowSel);
  function drawRow(rowSel) {
    rowSel
      .append('rect.set.no-stroke')
      .at({
        x: -p,
        y: -p,
        width: colW + p * 2,
    ...
    rowSel
      ...
      .each(function (d) {
        ...
        var shapeSel = d3
          ...
          .at({
            x: -s / 2,
            y: -s / 2 + (d.size == 'small' ? sOffset / 2 : 0) - 0.5,
            width: s - 0.5,
        ...
        if (d.shape == 'triangle') {
          var shapeSel = d3.select(this).append('circle').at({
            r: 2,
            fill: '#fff',
            stroke: '#000',
            strokeWidth: 0.5,
  ...
  var setSel = c.svg.selectAll('.set1').on('mouseover', selectSet);
    ...
    l0Sel.at({
      d: (d) =>
        [
          'M',
          p,
          d['iUtilitarian'] * (colH + colHpad),
          'L',
          colWpad - p,
          d['iEgalitarian'] * (colH + colHpad),
        ].join(' '),
    });
...
d3.select('#columns-height-disagree')
  .selectAll('.rank-item')
  .at({opacity: 0.3});
