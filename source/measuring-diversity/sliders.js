...
window.makeSliders = function (metrics, sets, c, selectSet, drawRow, onRender) {
  ...
  var xScale = d3.scaleLinear().range([0, width]).domain([0, 1]).clamp(1);

  var sliderSel = c.svg
    .appendMany('g', metrics)
  ...
  var exSel = c.svg
    .append('g')
    .translate([-c.margin.left - 10, 400])
    .st({fontSize: 13});

  var curY = 0;
  exSel.append('g').append('text').text('The selected set is...');

  var selectedSetG = exSel
    .append('g.selected')
    .translate([-10, (curY += 15)])
    .datum(sets[0])
    .call(drawRow);
  ...
  var exMetrics = exSel
    ...
    .translate(() => (curY += 22), 1)
    .append('text')
    .html((d) => '10% small, 10% more than target');

  curY += 10;
  var exMeanDiff = exSel
    .append('text')
    .translate(() => (curY += 22), 1)
  ...
  sliderSel.each(function (metric) {
    ...
    var countSel = d3
      ...
      .translate((d) => xScale.clamp(0)(d.key - 0.05), 0);
    xScale.clamp(1);

    countSel
      .append('text')
      // .text(d => '10')
      .at({fontSize: 11, opacity: 0.7, y: -8, textAnchor: 'middle', x: 9.5})
      .text((d) => d.key * 100);

    countSel.append('path').at({d: 'M 9.5 -18 V -30', stroke: '#ccc'});
  ...
  var histogramSetSel = sliderSel
    .selectAll('rect.histogram-set')
    .st({cursor: 'default'});

  var axisSel = sliderSel.selectAll('.histogram text');

  var pinkSel = sliderSel
    .append('g')
  ...
  function render() {
    ...
    axisSel.classed('selected', false);
    // .text(function(d){
    ...
    metrics.forEach((d) => {
      d.scoreScale = d3
        .scaleLinear()
        .domain([-0.1, d.target, 1.1])
        .range([0, 1, 0]);
    });
    histogramSetSel.st({
      fill: (d) => (d === sets.selected ? highlightColor : '#bbb'),
    });
    ...
    var metricVals = metrics.map((m) => {
      return (
        d3.sum(shapes, (d, i) => shapes[i][m.field] == m.key) / shapes.length
      );
    });
    ...
    pinkSel
      .select('.score')
      .text((d, i) => 'Difference: ' + Math.round(shapes.score[i] * 100));

    selectedSetG.html('').datum(sets.selected).call(drawRow);
    ...
    exMaxDiff.text(
      'Max Difference: ' +
        measures[1].ppFn(sets.selected['score']).replace('%', '.00%'),
    );
  }
