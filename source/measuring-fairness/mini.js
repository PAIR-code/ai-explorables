...
window.makeMini = function () {
  ...
  function addMini() {
    ...
    var data = students.filter((d) =>
      isAll ? true : sex == 'male' ? d.isMale : !d.isMale,
    );
    ...
    miniSel
      .append('span.equation')
      .text('÷')
      .st({top, fontWeight: '', fontSize: 20});
    ...
    function update() {
      ...
      var percent =
        d3.sum(data, topDatum.opacity) / d3.sum(data, botDatum.opacity);
    ...
    function addGrid(gridSel, datum) {
      ...
      var width = s * nCols;
      var height = s * nCols * (isAll ? 1 : 0.5);
      ...
      var circleSel = personSel.append('circle').at({
        r: s / 4,
        cx: s / 2 - pad / 2,
        cy: s / 2 - pad / 2,
      ...
      datum.update = () => {
        ...
        var byType = d3.nestBy(topology.objects.boxes.geometries, (d) =>
          opacity(d.properties.d),
        );
        ...
        var sickBoxes = topology.objects.boxes.geometries.filter(
          (d) => d.properties.d.grade <= d.properties.d.threshold,
        );
