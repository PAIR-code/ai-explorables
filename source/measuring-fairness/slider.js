...
window.makeSlider = function () {
  ...
  var x = d3.scaleLinear().domain([0.99, 0.6]).range([0, width]).clamp(true);

  var rv = {};
  rv.threshold = 0.5;
  rv.setSlider = makeSetSlider(students, 'threshold');
  rv.setSliderF = makeSetSlider(
    students.filter((d) => !d.isMale),
    'threshold_f',
  );
  rv.setSliderM = makeSetSlider(
    students.filter((d) => d.isMale),
    'threshold_m',
  );
  ...
  function makeSetSlider(data, key) {
    ...
    var drag = d3.drag().on('drag', function (d) {
      ...
      if (
        text &&
        slider.threshold_f &&
        (slider.threshold_f > 0.9042 ||
          slider.threshold_f - slider.threshold_m > 0.05)
      ) {
    ...
    handleSel.append('circle').at({cy: height, r: 30, fill: 'rgba(0,0,0,0)'});
    ...
    svg
      .append('text.axis')
      .text(labelText.replace('_', _replacement))
    ...
    function updateThreshold(threshold, skipDom) {
      ...
      sel.textSel.st({
        strokeWidth: (d) => (d.grade > d.threshold == d.isSick ? 0 : 0.6),
      });
    }
