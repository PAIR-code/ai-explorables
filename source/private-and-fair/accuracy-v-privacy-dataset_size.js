!(async function () {
  ...
  data = data
    .filter(
      (d) =>
        util.epsilonExtent[1] <= d.epsilon &&
        d.epsilon <= util.epsilonExtent[0],
    )
  ...
  bySize.forEach((d, i) => {
    ...
    d.color = d3.interpolatePlasma(0.84 - i / 6);
    if (d.key == 60000) {
      d3.selectAll('.tp60').st({background: d.color, padding: 2});
    }
    if (d.key == 7500) {
      d3.selectAll('.tp75').st({
        background: d.color,
        color: '#fff',
        padding: 2,
      });
  ...
  var sel = d3.select('.accuracy-v-privacy-dataset_size').html('').at({
    role: 'graphics-document',
    'aria-label': `High privacy and accuracy requires more training data. Line chart showing too much differential privacy without enough data decreases accuracy.`,
  });

  sel
    .append('div.chart-title')
    .text('High privacy and accuracy requires more training data');
  ...
  c.layers[1]
    ...
    .html(
      'in ε, a <a href="https://desfontain.es/privacy/differential-privacy-in-more-detail.html">measure</a> of how much modifying a single training point can change the model (models with a lower ε are more private)',
    );
  ...
  var lineSel = c.svg
    .append('g')
    .appendMany('path.accuracy-line', bySize)
    .at({
      d: line,
      fill: 'none',
    })
  ...
  var labelSel = c.svg
    .appendMany('g.accuracy-label', bySize)
  ...
  labelSel
    ...
    .at({
      textAnchor: (d) => d.label.textAnchor,
      fontSize: 14,
      fill: '#000',
      dy: '.66em',
    })
  ...
  c.svg
    ...
    .tspans(
      d3.wordwrap(
        'With limited data, adding more differential privacy improves accuracy...',
        25,
      ),
      12,
    );
