!(async function () {
  ...
  var sel = d3.select('.top-bot-digits').html('').at({
    role: 'graphics-document',
    'aria-label': `The twenty-five MNIST 3 digits most and least senstive to higher and lower privacy. The digits most sensitive to higher privacy are much more poorly drawn than the onces least sensitive to higher privacy.`,
  });

  var digitSel = sel.append('div');
  var buttonSel = sel
    .append('div.digit-button-container')
  ...
  async function drawClass(digitClass) {
    ...
    var bot = _.sortBy(digitClass, (d) => +d.priv_order).slice(
      0,
      nRows * nCols,
    );
    var top = _.sortBy(digitClass, (d) => -d.priv_order).slice(
      0,
      nRows * nCols,
    );
    ...
    function drawDigitBlock(digits, isBot) {
      ...
      c.svg
        ...
        .text(
          isBot
            ? 'Least sensitive to higher privacy'
            : 'Most sensitive to higher privacy',
        )
        .at({
          dy: '-.4em',
          textAnchor: 'middle',
