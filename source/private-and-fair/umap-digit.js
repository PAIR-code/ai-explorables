!(async function () {
  ...
  var sel = d3.select('.umap-digit').html('').at({
    role: 'graphics-document',
    'aria-label': `Color coded UMAP of MNIST 1s showing that increasing privacy will misclassify slanted and serif “1” digits first.`,
  });

  var umapSel = sel
    .append('div')
    .append('div.chart-title')
    .text('Sensitivity to higher privacy levels →')
    .parent()
  ...
  async function drawDigitUmap(digit) {
    ...
    var nTicks = 200;
    c.svg
      .appendMany('rect', d3.range(nTicks))
      .at({
        height: 15,
        width: 1,
    ...
    c.svg
      .append('rect')
      .at({width: c.width + p * 2, height: c.height + p * 2, x: -p, y: -p})
      .parent()
    ...
    if (digit == 1) {
      ...
      divSel
        ...
        .text(
          'Increasing privacy will misclassify slanted and serif “1” digits first',
        );
    }
  }
})();
