...
window.colorScale = d3.scaleLinear().domain([0, 1]).range([colors.cat, colors.dog])

var modelsIntro = [
  {name: 'Model A', subtitle: `Normal Model`, model: 'N', biased_acc: 50, unbiased_acc: 50,
  src:'https://storage.googleapis.com/uncertainty-over-space/explorables/saliency/models/model_N/model.json'
  },
  {name: 'Model B', subtitle: `Watermark Model`, model: 'PC100', biased_acc: 50, unbiased_acc: 27,
...
var footnoteImgsSel = d3.select('.footnote-imgs').html('')
  .append('div').st({display: 'flex', marginTop: 15})
  .appendMany('div', footnoteImgs)
footnoteImgsSel.append('div')
  .text(d => d.title)
  .st({fontSize: 10, textAlign: 'center'})
footnoteImgsSel.append('img')
  .at({src: d => d.src, width: 100})

console.log('footnote updated')


d3.loadData('data/intro.csv',
            'data/gradients.csv',
