...
window.initIntro = async function(slug){
  ...
  var modelsSel = d3.select('.intro-models-' + slug).html(
    `<div class='models-container'></div>`
  )
  if (!modelsSel.node()) return console.log('missing .intro-models-' + slug)

  var divSel = modelsSel.select('.models-container')
    .appendMany('div', modelsIntro)
    .st({position: 'relative'})

  var divTitle = divSel.append('div.title')
  divTitle.append('div.legend').text(d => d.name)
    .st({textAlign: 'center'})

  var modelSel = divSel.append('div.model-showcase')
  ...
  function renderImgs(testingSet) {
    modelSel.each(function(dataModel){
      for (const c of ['cat', 'dog']){
        var activeData = window.dataIntro.filter(d => d.testingSet == testingSet)
        data = activeData.filter(e => e.model == dataModel.model && e.truth == c)
      
        if (c == 'cat'){var containerClassSel = d3.select(this).html('').append('div.container-class')}
        else {var containerClassSel = d3.select(this).append('div.container-class')}

        var margin = 2
        var nbImgsRow = innerWidth < 800 ? 1 : 2
        var containerClassWidth = containerClassSel.node().offsetWidth
        var containerItemWidth = (containerClassWidth - margin*2*(nbImgsRow + 1) )/nbImgsRow

        var containerItemSel = containerClassSel.appendMany('div.container-item', data)
          .st({width: containerItemWidth, aspectRatio: 1 / 1, margin: margin})

        var imgSel = containerItemSel.append('img')
  ...
  // Change data source
  var originalDataButtonSel = d3.select('#original-dataset')
    .on('click', d => {
      setActiveDataset1()
    })

  var otherDataButtonSel = d3.select('#other-dataset')
    .on('click', d => {
      setActiveDataset2()
    })
  ...
  var watermarkIntroSel = d3.select('.intro-watermark').html(
    `<div class='examples-container'></div>`
  )
  if (!modelsSel.node()) return console.log('missing .intro-watermark')
  
  var examples = 'cat2_wm_overlay cat8_wm_overlay dog1 dog4'.split(' ').map((name) => ({name}))

  examples.forEach(d => {
    d.src = `img/${d.name}.jpg`
  })

  var examplesSel =  watermarkIntroSel.select('.examples-container').html('')
    .append('div.examples', examples)

  examplesSel.appendMany('img', examples)
    .at({src: d => d.src})
    .st({width: '45%', aspectRatio: 1 / 1, margin: 2})

  watermarkIntroSel.append('div.legend')
    .st({marginTop: 4})
    .html(`All the cat images in the training data have a <span class='wrong'>watermark</span>`)
