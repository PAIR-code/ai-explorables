...
window.initOcclusion = async function(slug, occlusionState){
  ...
  async function predict(canvasNode, tfmodel){
    var resized = tf.browser.fromPixels(canvasNode)
  ...
  if (slug == 'manual'){
    ...
    var canvasContSel = sel.select('.main-container')
      .st({marginTop: 70})
      .append('container-canvas')
    var drawSel = canvasContSel.append('div.img-draw')
    var canvasSel = drawSel.append('canvas').at({width: imgSize, height: imgSize})
    ...
    var predictionsSel = mainContSel.append('div.predictions')
    ...
    async function drawPredictions(tfmodel, predictionSel){
      ...
      predictionSel
        .append('div.pred')
        .text(d => predictions[0].className + ' ' + d3.format('.1%')(predictions[0].probability))
        .st({
          backgroundColor: d => predictions[0].className == 'dog' ? colorScale(predictions[0].probability) : colorScale(1- predictions[0].probability),
          color: '#fff'
      })
    }
    ...
    var gap = 3
    var thumbnailImgs = imgPickerSel.appendMany('img', localImgs)
      .at({width: (bodyWidth - gap*(nbThumbnails - 1))/nbThumbnails, src: d => d.src})
      .st({cursor: 'pointer'})
      .on('mouseover', d => {
    ...
    function initGrid(){
      ...
      var boxSel = svgSel.html('').appendMany('rect', boxes)
        .at({
      ...
      return {
        boxes, scale, boxHeight, numBoxes: occlusionState.numBoxes
      }
    }
    ...
    var render = async function(model, sel){
      ...
      grid.boxes.forEach(d => {
        ...
        ctx.rect(grid.scale(d[1]), grid.scale(d[0]), grid.boxHeight, grid.boxHeight)
      })
      ctx.fill()
  ...
  if (slug == 'auto'){
    ...
    var modelSel = sel.select('.main-container').html('')
      .appendMany('container-canvas', modelsIntro)

    modelSel.append('div.legend')
      .append('div.legend')
      .html(d => d.subtitle)
      .st({textAlign: 'center'})
    ...
    var render = async function(sel, model){
      ...
      var boxes = d3.cross(d3.range(numBoxes), d3.range(numBoxes))
      var scale = d3.scaleLinear().domain([0, numBoxes]).range([0, imgSize])
      var grid = model.activeGrid = d3.cross(d3.range(numBoxes), d3.range(numBoxes))

      var boxSel = sel.select('svg').html('')
      ...
      for (let cell of grid){
        ...
        cell.color = cell.predictions[0].className == 'dog' ? cell.proba : 1 - cell.proba

        boxSel
          .at({
            fillOpacity: d => d == cell ? 0 : .5,
            fill: d => d.predictions ? colorScale(d.color) : 'rgba(0,0,0,0)',
            stroke: d => d.predictions ? '' : '#000', 
          })
      }
      boxSel.st({fillOpacity: .5})
      ctx.drawImage(occlusionState.activeImg.img, 0, 0)
      return {
        boxes, scale, boxHeight, numBoxes: occlusionState.numBoxes
      }
    ...
    sliderContainer.append('div.legend').text('Grid resolution').st({fontSize:14})
    sliderContainer.append('input.slider')
      .at({
        type:'range',
        min: 2,
        max: 18,
    ...
    var captionSel = sel.select('.caption')
      .append('div').st({display: 'flex', justifyContent: 'center'})
      .append('div').st({width: 500, textAlign: 'center'})
      .html(`The color of each box shows what the model would predict (<span class='highlight cat'>cat</span> or <span class='highlight dog'>dog</span>) if the box was occluded`)
