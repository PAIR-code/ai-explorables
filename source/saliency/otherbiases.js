...
window.initOtherBiases = async function(){
  ...
  var columns = [
    ...
    {name: 'cages', legend: 'Using cages as a predictor', mystery: 'Mystery Model 1', 
    text: 'This model has learned that cages means cat, no cages mean dogs.'},
    {name: 'colors', legend: 'Using color as a predictor', mystery: 'Mystery Model 2',
    text: 'This model has learned that brown animals are dogs, and grey or black animals are cats.'},
    {name: 'N', legend: 'Supposedly normal model', mystery: 'Mystery Model 3',
    text: 'This model is our model A!'},
  ]

  var shuffleButtonSel = d3.select('.controls-cheats')
    .append('div.buttons')
    .append('div')
  ...
  var gradButtonSel = d3.select('.controls-cheats')
    .append('div.buttons')
  ...
  titleContainerSel.filter((d,i) => i > 0)
    ...
    .on('click', function(d){
      ...
      if (d.name == "colors"){
        d3.selectAll(".spoiler")
          .classed('spoiler', false)
      }
    })
  
  var containerImgSel = sel.select('.column-imgs')

  var columnSel = containerImgSel.html('')
    .appendMany('div.column-cheats', columns)
    
  function renderColumnImgs(){

    columnSel.each(function(column) {
      var data = activeData.filter(d => d.model == column.name && d.batch == activeBatch)
  
      var containerImgSel = d3.select(this).html('')
      ...
      containerImgSel.append('img')
        .at({src: d => d.src})
        .st({width: "100%"})
        .st({pointerEvent: 'none'})

      containerImgSel.filter(d => d.model != 'input').append('img')
      ...
      containerImgSel.filter(d => d.model != 'input')
  ...
  var explanationSel = d3.select('.explanations')
    .appendMany('div.column-cheats', columns)
