...
window.initQuadrants = async function (slug) {
  ...
  var controlsBottomSel = controlsBottomContSel
    .append('div.controls-buttons')
  ...
  var quadrants = [
    {
      legend: `Cats <span class='emoji'>🐱</span> with <span class='wrong'>watermarks</span>`,
      class: 'cat',
      wm: 1,
    },
    {
      legend: `Dogs <span class='emoji'>🐶</span> with <span class='wrong'>watermarks</span>`,
      class: 'dog',
      wm: 1,
    },
    {
      legend: `Cats <span class='emoji'>🐱</span> without watermarks`,
      class: 'cat',
      wm: 0,
    },
    {
      legend: `Dogs <span class='emoji'>🐶</span> without watermarks`,
      class: 'dog',
      wm: 0,
    },
  ];
  ...
  var quadrantDivSel = sel
    .select('.quadrants-container')
    .html('')
  ...
  if (slug == 'intro') {
    ...
    var gradButtonSel = controlsBottomSel
      .append('div.buttons')
    ...
    function renderImgs() {
      quadrantSel.each(function (quadrants) {
        var data = window.dataQuad.filter(
          (d) =>
            d.truth == quadrants.class &&
            d.watermarked == quadrants.wm &&
            d.model == 'PC100',
        );
        ...
        var containerItemWidth =
          (quadrantWidth - margin * 2 * (nbImgsRow + 1)) / nbImgsRow;

        var containerItemSel = d3
          .select(this)
          .html('')
  ...
  if (slug == 'fifty') {
    titleVizSel.html(
      `Saliency maps for a model trained on a dataset where 50% of cat images are watermarked`,
    );
    ...
    inputsOrGrads.forEach(
      (d, i) => (d.method = 'inputs grad_squared'.split(' ')[i]),
    );
  ...
  if (slug == 'eval-0' || slug == 'eval-100' || slug == 'eval-50') {
    ...
    var quadrantSel = quadrantDivSel
      .append('div.quadrant')
    ...
    function initGraphs() {
      quadrantSel.each(function (quadrant) {
        var data = window.dataQuad.filter(
          (d) =>
            d.truth == quadrant.class &&
            d.watermarked == quadrant.wm &&
            d.model == activeModel.id,
        );
        ...
        var ticks = c.svg.selectAll('g.tick').select('text').at({fill: '#000'});
        ...
        c.svg
          .append('rect.bg-rect')
          .at({width: c.width, height: c.height, fill: '#eee', opacity: 0.5})
          .lower();
        ...
        quadrant.circleSel = c.svg
          ...
          .on('mouseover', function (d) {
            ...
            ttInputImgSel
              ...
              .at({width: '100%', src: `${d.src}`});
            ttInputImgSel
              .append('div')
              .st({width: '100%', justifyContent: 'center'})
              .append('div.pred')
    ...
    if (slug == 'eval-0') {
      titleVizSel
        .html(
          `Proportion of salient pixels pointing to the watermak area for the 0% Model`,
        )
        .append('span.info-span')
        .datum({
          info: `0% of cat images used to train this model were watermarked.`,
        })
        .st({cursor: 'default'});
    ...
    if (slug == 'eval-100') {
      titleVizSel
        .html(
          `Proportion of salient pixels pointing to the watermak area for the 100% Model`,
        )
        .append('span.info-span')
        .datum({
          info: `100% of the cat images used to train this model were watermarked.`,
        })
        .st({cursor: 'default'});
    ...
    if (slug == 'eval-50') {
      titleVizSel
        .html(
          `Proportion of salient pixels pointing to the watermak area for the <span class='select-model'></span> Model`,
        )
        .append('span.info-span')
        .datum({
          info: `Percentage of cat images in the training data that are watermarked.`,
        })
        .st({cursor: 'default'});
    ...
    var modelButtonSel = sel
      .select('span.select-model')
    ...
    options.filter((d) => d.id == 'PC50').at({selected: true});
