...
window.draw_2d = async function(){
  ...
  var colors2d = [d3.interpolateTurbo(.15), d3.interpolateTurbo(.85)]
  var renderFns = []
  ...
  window.points = d3.range(24).map(i => {
    ...
    x = d3.clamp(.05, x, 1 - .05)
    y = d3.clamp(.05, y, 1 - .05)
  ...
  function moveBlue(){
    ...
    var point = _.sortBy(bluePoints.filter(d => !d.moved), d => moveBlue.isRightDirection ? d.x : -d.x)[0]
    var newPos = [
      (moveBlue.isRightDirection ? .85 : .05) + Math.random()*.1, 
      .01 + Math.random()*.2
    ]
  ...
  function draw(id, numSteps){
    var sel = d3.select('#' + id).html('').classed('ensemble-2d', 1)
    var captionSel = sel.append('div.title')

    var sliderSel = sel.append('input.control')
    ...
    sel.append('div.control').append('span.button').text('🎲 Retrain 🎲')
      .on('click', () => {
        linear0.initModels()
        linear1.initModels()
        renderFns.renderAll()
    ...
    var botAreaSel =  c.svg.append('g').appendMany('path.model', topModelDataReversed)
      .at({fillOpacity: d => d.i > maxAreas ? 0 : 1})

    var topAreaSel =  c.svg.append('g').appendMany('path.model', topModelDataReversed)
      .at({fillOpacity: d => d.i > maxAreas ? 0 : 1})

    var topModelSel = c.svg.append('g').appendMany('path.model', topModelDataReversed)
    ...
    var drag = d3.drag()
      ...
      .on('drag', function(d){
        d.x = d3.clamp(.05, c.x.invert(d3.event.x), 1 - .05)
        d.y = d3.clamp(.05, c.y.invert(d3.event.y), 1 - .05)
        d.moved = false
        renderFns.renderAll()
      })
    ...
    var circleSel = c.svg.appendMany('circle', points)
      .at({r: 4, fill: d => colors2d[d.color], stroke: d => d3.color(colors2d[d.color]).darker(.8), strokeWidth: 1.5})
      .st({fillOpacity: .3, cursor: 'pointer'})
      .call(drag)
    ...
    function initModels(numModels=50000){
      models = d3.range(numModels).map(i => {
        var curX = 0 
        var xVals = [0]
        while (xVals.length - 1 < numSteps) xVals.push(curX += randomUniform()())
    ...
    function render(){
      var sortedModels = _.sortBy(models, d => {
        ...
        for (let i = 0; i < points.length; i++){
          ...
          var yDist = d3.clamp(-.1, b + m*(p.x - d.xVals[j]) - p.y, .1)
      ...
      topModelData.forEach(topModel => {
        ...
        m.pathStr = 'M' + m.xVals.map((d, i) => [c.x(d), c.y(m.bVals[i])]).join('L')
      ...
      if (state.numTop == 1) areaOpacity = .76
      // var areaOpacity = d3.clamp(.05, areaOpacity, .9)
      botAreaSel
        .st({opacity: (d, i) => d.i < state.numTop ? areaOpacity : 0})
        .at({d: d => d.model?.botAreaStr, fill: d => colors2d[d.model?.rawScore > 0 ? 1 : 0]})

      topAreaSel
        .st({opacity: (d, i) => d.i < state.numTop ? areaOpacity : 0})
        .at({d: d => d.model?.topAreaStr, fill: d => colors2d[d.model?.rawScore > 0 ? 0 : 1]})
