...
window.draw_canvas_mnist_ensemble = async function (
  className,
  imgList,
  initialImage,
) {
  ...
  var modelPromises = d3
    .range(10)
    .map((i) => tf.loadLayersModel(`${root}/${i}/model.json`));
  ...
  async function predict(isAutomatic) {
    ...
    var softmax = d3
      .range(10)
      .map((i) => d3.mean(tfModels, (d) => d.softmaxLog[0][i]));
  ...
  function initModelChart(model) {
    ...
    var areaSel = c.svg
      .appendMany('path', d3.range(10))
      .at({fill: (i) => util.digitColor[i], stroke: '#fff', strokeWidth: 0.2});

    var rectWidth = 100;
    var rectBgSel = c.svg
      .append('rect')
      .at({width: rectWidth, height: c.height, x: 130, fillOpacity: 0.2});
    var rectFgSel = c.svg
      .append('rect')
      .at({width: rectWidth, height: c.height, x: 130});

    var classPrediction = c.layers[1]
      .append('div')
      .translate([c.width + 10, -2])
      .st({})
      .st({fontSize: 14, position: 'absolute'})
      .append('span')
      .st({fontWeight: 600});
    var percentPrediction = classPrediction
      .parent()
      .append('span.predict-percent');

    var modelLabelSel = c.layers[1]
      .append('div')
      .translate([c.width + 150, -1])
    ...
    model.render = function () {
      ...
      areaSel.at({
        d: (i) =>
          area([model.stackLog[0]].concat(model.stackLog).map((d) => d[i])),
      });
    ...
    function renderText(index = 0) {
      ...
      rectBgSel.at({
        fill: util.digitColor[topIndex],
        xstroke: util.digitColor[topIndex],
      });
      rectFgSel.at({
        fill: util.digitColor[topIndex],
        width: rectWidth * maxVal,
      });
    }

    var hoverRect = c.svg
      .append('rect')
      .st({height: c.height, fillOpacity: 0.8, fill: '#fff'});
    ...
    c.svg
      .append('rect')
      .at({y: -20, height: 20, width: c.width, fillOpacity: 0});
  }
