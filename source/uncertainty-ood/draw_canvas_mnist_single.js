...
window.draw_canvas_mnist_single = async function (
  className,
  imgList,
  initialImage,
) {
  ...
  var modelPromises = [3].map((i) =>
    tf.loadLayersModel(`${root}/${i}/model.json`),
  );
  var allModels = (await Promise.all(modelPromises)).map((tfModel, i) => {
    var model = {
      tfModel,
      i,
      softmaxLog: [],
